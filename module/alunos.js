...
const studentSelect = (idEnroll) =>{
    //variavel paara listar os estudantes
    let listStudents = []
    //variavel para ter o numero da matricula
    let numberRegister = idEnroll;
    //variavel de erro para true
    let erro = true;
    //percorrer o json
    let student = [];

    //se o numero da matricula der indefinido 
    if(typeof(numberRegister)!= 'undefined'){
        studentSelect.forEach(item => {
            if(item.matricula.toLowerCase().includes(numberRegister.toLowerCase())){
                student.photoStudent = item.foto
                student.nameStudnet = item.nome
                student.registrationName = item.matricula
                student.genreStudent = item.sexo
